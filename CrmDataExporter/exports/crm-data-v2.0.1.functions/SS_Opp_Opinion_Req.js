function SS_Opp_Opinion_Req(pURL,pData)
{
	...
	//var result = "trying to lunch ST webservice";
	var result = oResponse.StatusCode;
	...
	if (oResponse.StatusCode == 200 || oResponse.StatusCode == "True" || oResponse.StatusCode == "OK" ) {
	    try {
	        oStream = oResponse.GetResponseStream();
	        oStreamReader = new System.IO.StreamReader(oStream, System.Text.Encoding.UTF8);
	        var result = oStreamReader.ReadToEnd();
	...
	//var vReqInsertLog = "insert into  (xmethod, xappel, xretour, xdebug, xdate_log) values('ZCrmDa11Davis', '" + pData.replace(/'/g, "''") + "', '" + result.replace(/'/g, "''") + "', '', getdate()) ";
